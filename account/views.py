...
def user_orders(request):
    ...
    return render(request, 'account/dashboard/user_orders.html', {'section': 'profile', 'orders': orders, 'addresses': addresses})

@login_required
def change_order_status(request):
    if request.POST.get('action') == 'get':
        order_id = request.GET.get('order_id')
        Order.objects.filter(pk=order_id).update(order_recieved=True)
